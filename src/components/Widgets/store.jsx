...
const WidgetStore = ({ show, colaborador }) => {
  ...
  return (
    <div
      className={`border-2 p-3 absolute top-[8%] rounded-[60px] items-center justify-center ite h-[85%] w-[90%] border-gray-100 bg-white shadow-lg flex transition-all duration-300 ${mostrar
        ...
        <div className="flex py-5 flex-col overflow-y-auto items-center justify-start w-full h-full ">
          {options.map((option, index) => (
            <button
              ...
              key={index}
              className={`transition-all duration-300 flex py-3 items-center justify-center text-gray-700 rounded-xl my-4 h-12 w-[88%] border-gray-100 shadow-lg ${selectedOption === option
